Live link : https://sai-aishwarya-codes.github.io/my-portfolio/ 

<img width="1886" height="843" alt="screen" src="https://github.com/user-attachments/assets/21067a98-15dc-4141-a7d8-66abd1ffa71d" />

This repository contains a fully customized, responsive resume website built using the StartBootstrap Resume Theme. The template has been redesigned to represent my academic profile, technical skillset, and project experience in Artificial Intelligence, Machine Learning, Data Science, and Full-Stack Development.
This portfolio is deployed using GitHub Pages and serves as an online version of my resume.
Features
•	Responsive design optimized for mobile, tablet, and desktop
•	Clean sidebar navigation for quick section switching
•	One-page layout for smooth scrolling experience
Fully customized content including:
•	About
•	Education
•	Technical Skills
•	Projects
•	Achievements
•	Lightweight and fast — built with HTML, CSS, JS, and Bootstrap
Technologies Used Frontend
•	HTML5
•	CSS3
•	Bootstrap 5
•	JavaScript
•	Font Awesome Icons
•	Google Fonts
•	Tools & Workflow
•	Git & GitHub
GitHub Pages for deployment
Technical Overview
1.	Bootstrap-Based Layout
The project uses StartBootstrap Resume, which provides:
•	A fixed left sidebar (.navbar)
•	Smooth-scroll navigation
•	Section-based content blocks
•	Predefined typography and spacing
2.	Custom Modifications
•	customized the layout to include updated sections reflecting my resume:
•	Removed default sections (Interests, Awards)
•	Added Projects and Achievements sections
•	Updated content with professional details
•	Improved formatting & readability
3.	Responsive Behavior
•	The design is adapted using:
•	container-fluid
•	resume-section classes
•	Bootstrap grid system
•	Responsive images using img-fluid
4.	SEO & Metadata (Optional Enhancements)
•	Metadata tags can be added to improve search visibility:
License
This project uses the MIT License included in the StartBootstrap theme.
Author
Sai Aishwarya Reddy Devarapalli 📍 Arizona, USA 📧 sdevar24@asu.edu
🔗 https://www.linkedin.com/in/sai-aishwarya
🔗 https://github.com/sai-aishwarya-codes



